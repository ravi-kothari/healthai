...
async def analyze_symptoms(
    ...
    **Example Request:**
    ...
    ```
    """
    logger.info(f"Symptom analysis requested by user {current_user.id}")
    ...
    try:
        ...
        logger.info(f"Symptom analysis completed: urgency={analysis.urgency}, triage_level={analysis.triage_level}")
...
async def triage_assessment(
    ...
    **Example Request:**
    ...
    ```
    """
    logger.info(f"Triage assessment requested by user {current_user.id}")
    ...
    try:
        ...
        logger.info(f"Triage assessment completed: level={assessment.triage_level}, urgency={assessment.urgency}")

        # Log emergency cases
        if assessment.triage_level == 1:
            logger.warning(f"EMERGENCY triage for user {current_user.id}: {assessment.emergency_flags}")
...
async def generate_questionnaire(
    ...
    **Example Response:**
    ...
    ```
    """
    logger.info(f"Questionnaire generation requested by user {current_user.id}")
    ...
    try:
        ...
        logger.info(f"Generated {len(questions)} questions for chief complaint: {request.chief_complaint}")
...
@router.get("/{patient_id}/patient-summary", response_model=PatientSummaryResponse)
async def get_patient_summary(
    ...
    **🎯 Patient-Facing API Firewall Endpoint**

    This endpoint combines PreVisit.ai symptom analysis with Appoint-Ready context
    to create a seamless patient experience. It transforms clinical data into
    ...
    **Example Request:**
    ```
    GET /api/previsit/2b7ed7f3-c480-49ba-ad9c-07073e6ca46a/patient-summary
    ```
    ...
    **Example Response:**
    ...
    ```
    """
    logger.info(f"Patient summary requested for {patient_id} by user {current_user.id}")
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Could not fetch full provider context: {e}")
        ...
        if provider_context:
            try:
                ...
                logger.warning(f"Could not calculate risk scores: {e}")
        ...
        if include_appointment_prep and provider_context:
            try:
                ...
                logger.warning(f"Could not detect care gaps: {e}")
        ...
        logger.info(f"Generated patient summary with {len(patient_summary.topics_to_discuss)} topics")
...
@router.get("/health")
async def previsit_health_check():
    """
    Health check endpoint for PreVisit.ai services.
