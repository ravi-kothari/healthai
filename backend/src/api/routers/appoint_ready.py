"""
Appoint-Ready API endpoints.

Provider-facing appointment preparation and clinical decision support.
"""
...
async def get_appointment_context(
    ...
    **Example Request:**
    ```
    GET /api/appoint-ready/context/2b7ed7f3-c480-49ba-ad9c-07073e6ca46a?include_fhir=true
    ```
    ...
    **Example Response:**
    ...
    ```
    """
    logger.info(f"Appointment context requested for patient {patient_id} by user {current_user.id}")
...
async def appoint_ready_health_check():
    """
    Health check endpoint for Appoint-Ready services.

    Returns status of all Appoint-Ready components.
    """
    return {
        "status": "healthy",
        "services": {
